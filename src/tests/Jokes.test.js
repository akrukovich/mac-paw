...
describe("Joke tests", () => {
  const jokesAmount = () =>
    JSON.parse(localStorage.getItem("favoriteJokes")).length;
  it("Random jokes test", () => {
    act(() => {
    ...
    expect($(".timestamp").text()).toStrictEqual(
      expect.stringContaining(hoursAgo.toString())
    );
  });
  ...
  it("heartToggle test ", () => {
    ...
    act(() => {
      render(
        <Joke reRenderHeartChange={reRenderHeartChange} joke={randomJoke} />,
        container
      );
    });
